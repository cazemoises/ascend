...
The judge worker is a background process that dequeues code submissions from a Redis list, executes them in an isolated Docker sandbox, evaluates the output against test cases, and writes the verdict back to the database. It is implemented in Go.
