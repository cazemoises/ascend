...
-- rather than pointing at one language's syntax.
--
...
UPDATE challenges SET description = $DESC_1$Leia uma linha de texto. Para cada letra, inverta maiúscula↔minúscula (números e símbolos ficam iguais). Não use funções prontas de inversão de caixa da sua linguagem (como .swapcase() em Python) — faça o loop manualmente.$DESC_1$
WHERE slug = 'trilha-n1-inverte-caixa';

-- trilha-n1-area-circulo
UPDATE challenges SET description = $DESC_2$Leia o raio r. Imprima a área do círculo (π × r²) com 2 casas decimais. Use a constante de PI da sua linguagem (math.pi em Python, Math.PI em Java/JavaScript, math.Pi em Go).$DESC_2$
